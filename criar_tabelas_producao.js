...
async function criarTabelas() {
    try {
        console.log('Criando tabelas de produção científica no PostgreSQL...');
        ...
        // 4. Produção Técnica e Tecnológica
        await pool.query(`
        ...
        if (parseInt(resPub.rows[0].count) === 0) {
            console.log('Populando dados iniciais de Publicações...');
            await pool.query(`
        ...
        if (parseInt(resAcad.rows[0].count) === 0) {
            console.log('Populando dados iniciais de Produção Acadêmica...');
            await pool.query(`
        ...
        if (parseInt(resPrem.rows[0].count) === 0) {
            console.log('Populando dados iniciais de Prêmios...');
            await pool.query(`
        ...
        if (parseInt(resTec.rows[0].count) === 0) {
            console.log('Populando dados iniciais de Produção Tecnológica...');
            await pool.query(`
        ...
        if (parseInt(resOut.rows[0].count) === 0) {
            console.log('Populando dados iniciais de Outras Produções...');
            await pool.query(`
        ...
        console.error('Erro ao configurar tabelas de produção científica:', err);
    } finally {
        pool.end();
    }
}
